ingredient_info = {
    ...
    "soy lecithin": "Common emulsifier. May be an allergen for soy-sensitive users.",
    "sugar": "Added sugar that can increase calorie intake and blood glucose.",
    "cocoa butter": "High-fat ingredient commonly found in chocolate.",
}
...
def analyze_ingredients(ingredients_text):
    ...
    for ingredient, description in ingredient_info.items():
        if ingredient in ingredients_lower:
            results.append(f"⚠ {ingredient.title()}: {description}")
    ...
    for sugar in hidden_sugars:
        if sugar in ingredients_lower:
            results.append(f"🍬 Hidden Sugar Detected: {sugar.title()}")
